        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              if (widget.userCollections.userCollectionsList.data.isEmpty)
                Padding(
                  padding: const EdgeInsets.all(8.0),
                  child: Text(
              ...
              if (_loading == false &&
                  widget.userCollections.userCollectionsList.data.isNotEmpty)
                DropdownButton<String>(
                  value:
                      dropDown == ' ' ? widget.collectionNames.first : dropDown,
                  iconSize: 24,
                  elevation: 16,
                  underline: Container(
                    height: 2,
                    ...
                    : () async {
                        if (_formKey.currentState!.validate()) {
                          ...
                          String result = await NewTokenAndCollection.newTokken(
                              ...
                              widget.userCollections.userCollectionsList.data
                                  .firstWhere(
                          ...
                          if (result == '201') {
                            var selectedCollection = widget
                                .userCollections.userCollectionsList.data
                                .firstWhere(
                              (element) => element.name == dropDown,
                            );
                            var newToken = Datum(
                              ...
                              currentUserData: CurrentUserData(isLiked: false),
                              price: Price(
                                  value: _priceController.text, unit: 'ether'),
                            );

                            final allTokkens =
                                Provider.of<AllTokens>(context, listen: false);
                            allTokkens.addToken(newToken);

                            //allTokkens.tokken.data.add(newToken);
                            //allTokkens.acquiredTokens.data.add(newToken);

                            // await allTokkens
                            //     .getAllTokens()
                            //     .whenComplete(() => null);
                            // await allTokkens
                            //     .getDownloadedtokens()F
                            //     .whenComplete(() => null);
                            // await allTokkens
                            //     .getUploadedtokens()
                            //     .whenComplete(() => null);
